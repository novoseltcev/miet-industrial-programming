...
rack::Track(Car car1, Car car2, Train train1, int volume) {
...
Track Order::best(std::string startP, std::string finishP, Type delivery, int volume) {
	int start = decr(startP);
	int finish = decr(finishP);
	int** mat = new int*[SIZE];
	for (int i = 0; i < SIZE; i++)
		mat[i] = new int[SIZE];
	mat = matrixUpd(delivery);
